#include <UmathTest.h>
#include <approx.h>
#include <umath/Vector.h>
#include <cmath>

using namespace UMath;

const float defaultApproxDelta = 0.000001;

TEST_F(UmathTest, DataAlias)
{
    Vec3f v1;
    ASSERT_TRUE(v1.x == 0);
    ASSERT_TRUE(v1.y == 0);
    ASSERT_TRUE(v1.z == 0);

    Vec3f v2(42);
    ASSERT_TRUE(v2.data[0] == 42);
    ASSERT_TRUE(v2.data[1] == 42);
    ASSERT_TRUE(v2.data[2] == 42);

    Vec3f v3(1, 2, 3);
    ASSERT_TRUE(v3.x       == 1);
    ASSERT_TRUE(v3.data[1] == 2);
    ASSERT_TRUE(v3.z       == 3);
}

TEST_F(UmathTest, ConstBinaryOperations)
{
    ASSERT_EQ(Vec3f(1, 2, 3) + Vec3f(3, 2, 1)   , Vec3f(4));
    ASSERT_EQ(Vec3f(3, 2, 1) - Vec3f(0, -1, -2) , Vec3f(3));
    ASSERT_EQ(Vec3f(1, 2, 3) * Vec3f(6, 3, 2)   , Vec3f(6));

    ASSERT_EQ(Vec3f(1, 2, 3) % Vec3f(6, 3, 2)   , 18);
    ASSERT_EQ(Vec3f(1, 2, 3) ^ Vec3f(3, 2, 1)   , Vec3f(-4, 8 , -4));

    ASSERT_EQ(Vec3f(1, 2, 3) * 2.0f             , Vec3f(2, 4, 6));
    ASSERT_EQ(2.0f * Vec3f(1, 2, 3)             , Vec3f(2, 4, 6));

    ASSERT_EQ(Vec3f(2, 4, 6) / 2                , Vec3f(1, 2, 3));

}

TEST_F(UmathTest, MutBinaryOperations)
{
    Vec3d v(1, 2, 3);

    v += Vec3d(2, 4, 6);
    ASSERT_EQ(v, Vec3d(3, 6, 9));

    v -= Vec3d(1, 2, 3);
    ASSERT_EQ(v, Vec3d(2, 4, 6));

    v *= Vec3d(0.5, 0.25, 1.5);
    ASSERT_EQ(v, Vec3d(1, 1, 9));

    v /= 0.5;
    ASSERT_EQ(v, Vec3d(2, 2, 18));

    v *= 0.5;
    ASSERT_EQ(v, Vec3d(1, 1, 9));

    v ^= Vec3d(2, 1, 3);
    ASSERT_EQ(v, Vec3d(-6, 15, -1));
}

TEST_F(UmathTest, ConstMethods)
{
    Vec3f v(1, 2, 3);
    ASSERT_EQ(v.x_axis() , Vec3f(1, 0, 0));
    ASSERT_EQ(v.y_axis() , Vec3f(0, 2, 0));
    ASSERT_EQ(v.z_axis() , Vec3f(0, 0, 3));
    ASSERT_EQ(v.len()    , static_cast<float>(sqrt(14.0f)));
}

TEST_F(UmathTest, MutMethods)
{
    Vec3f v(1, 2, 3);
    auto len = static_cast<float>(sqrt(14.0f));
    ASSERT_EQ(v.normalize()  , Vec3f(1.0f / len, 2.0f / len, 3.0f / len));
    ASSERT_TRUE(TestUtils::CompareApprox(v.len(), 1.0f, defaultApproxDelta));
}